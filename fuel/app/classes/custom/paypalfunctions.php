<?php
	...
	********************************************/
	...
	//'------------------------------------
	$API_UserName = "hling_1358162204_biz_api1.encrenomade.com";
	$API_Password = "1358162227";
	$API_Signature = "A0BExctOvTC4SbLvY4TjpGYIJLIMAYGNVdRR4J29yUe6bh2wjtZyq9pY";
	...
	*/
	function hash_call($methodName, $nvpStr)
	{
		...
		$config = Config::get('custom.paypal');
		...
		$nvpreq="METHOD=" . urlencode($methodName) . "&VERSION=" . urlencode($config['version']) . "&PWD=" . urlencode($config['API_Password']) . "&USER=" . urlencode($config['API_UserName']) . "&SIGNATURE=" . urlencode($config['API_Signature']) . $nvpStr . "&BUTTONSOURCE=" . urlencode($config['sBNCode']);
	...
	*/
	...
	{
		$PAYPAL_URL = Config::get('custom.paypal.PAYPAL_URL');
	...
	{
		$PAYPAL_DG_URL = Config::get('custom.paypal.PAYPAL_DG_URL');
